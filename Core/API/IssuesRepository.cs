...
namespace GithubSharp.Core.API
{
    public class IssuesRepository : Base.BaseApi
    {
        ...
        {
            var state = issueState == IssueState.Open ? "open" : "closed";
            LogProvider.LogMessage(string.Format("IssuesRepository.List - repository : '{0}', Username : '{1}', issueState : '{2}'",
                repository, username, state));
            var url = GetPathByDefault(repository, username, "?state={0}", state);
        ...
        {
            LogProvider.LogMessage(string.Format("IssuesRepository.View - repository: '{0}', Username : '{1}', Id : '{2}'", repository, username, id));
            var url = GetPathByDefault(repository, username, "/{0}", id);
            var result = ConsumeJsonUrl<IssueResponse>(url);
            return result;
        }
        ...
        {
            LogProvider.LogMessage(string.Format("IssuesRepository.Labels - Labels : repository '{0}', Username : '{1}', IssueId : '{2}'",
                                                 repository, username, id));
            var url = GetPathByDefault(username, repository, "/{0}/labels", id);
        ...
        {
            LogProvider.LogMessage(string.Format("IssuesRepository.Comments - repository: '{0}', Username : '{1}', Id : '{2}'", repository, username, id));
            var url = GetPathByDefault(username, repository, "/{2}/comments", id);
